#!/usr/bin/env python3
"""
Conversation Export Hook (/copy:this command)

A UserPromptSubmit hook that intercepts /copy:this commands, parses the transcript
JSONL directly, and delivers results to clipboard or file.

Usage:
    /copy:this              → copy last assistant response to clipboard
    /copy:this 2            → copy 2nd most recent response
    /copy:this -3           → copy last 3 assistant responses
    /copy:this -p 3         → copy last 3 rounds (user + assistant pairs)
    /copy:this --all        → copy full conversation
    /copy:this list [N]     → list recent N responses (default 10)
...
def parse_args(prompt: str) -> Optional[Args]:
    """Parse /copy:this command arguments. Returns None if prompt doesn't match."""
    prompt = prompt.strip()

    match = re.match(r"^/copy:this(?:\s+(.*))?$", prompt)
    if not match:
        return None
    ...
    if not rest:
        return args  # /copy:this → copy last response
...
def display_help():
    ...
    print("""Usage: /copy:this [options]

Copy & export Claude responses from the current session.

  /copy:this              Copy last response to clipboard
  /copy:this 2            Copy 2nd most recent response
  /copy:this -3           Copy last 3 responses
  /copy:this -p 3         Copy last 3 rounds (prompts + responses)
  /copy:this --all        Copy full conversation
  /copy:this list [N]     List recent N responses (default 10)
  /copy:this find "term"  Search responses
...
def main():
    ...
    if args.mode == "find":
        if not args.search_term:
            print("Usage: /copy:this find \"search term\"", file=sys.stderr)
            block_with_stderr()
